...
**/
	if(RootPtr != NULL){
		...
		while(!SEmptyP(&zasobnik)){
			...
			Leftmost_Preorder(tmp->RPtr, &zasobnik);
		}
	}
...
**/
	...
	if(RootPtr != NULL){
		...
		while(!SEmptyP(&zasobnik)){
			tBTNodePtr tmp = STopPopP(&zasobnik); 
			BTWorkOut(tmp);
...
**/
	...
	Leftmost_Postorder(RootPtr, &zasobnik_p, &zasobnik_b);
	while(!SEmptyP(&zasobnik_p)){
			RootPtr = STopPopP(&zasobnik_p); 
			SPushP(&zasobnik_p, RootPtr);
			if (STopPopB(&zasobnik_b)) { 
				SPushB(&zasobnik_b, false); 
				Leftmost_Postorder(RootPtr->RPtr, &zasobnik_p, &zasobnik_b);
			} else { 
				STopPopP(&zasobnik_p);
				BTWorkOut(RootPtr);
			}
